...
// ----------------------------------------------------------------------------
Collection<Sentence> sents = jcas.select type:Sentence, filter:contains(NamedEntityMention)
...
jcas.select(type:PseudoNegationTrigger).each { PseudoNegationTrigger pseudo ->
    jcas.select(type:NegationTrigger, filter:coveredBy(pseudo)).each { NegationTrigger neg ->
...
// ----------------------------------------------------------------------------
jcas.select(type:NegationScope, filter:contains(NamedEntityMention)).each { NegationScope scope ->
    jcas.select(type:PreNegationTrigger, filter:coveredBy(scope)).each { PreNegationTrigger trigger ->
        jcas.select(type:NamedEntityMention,
        filter:and(coveredBy(scope), after(trigger.end))).each { NamedEntityMention mention ->
            mention.polarity = -1
        }
    }
    jcas.select(type:PostNegationTrigger, filter:coveredBy(scope)).each { PostNegationTrigger trigger ->
        jcas.select(type:NamedEntityMention,
        filter:and(coveredBy(scope), before(trigger.begin))).each { NamedEntityMention mention ->
            mention.polarity = -1
        }
    }
}
